/* patch_pages.js —— ui.js 三页注册+index.html 容器按钮（用后即删） */
'use strict';
const fs = require('fs');
let n = 0;
...
/* 1) index.html：page-root 容器 + 两个 script + 右栏地图按钮 */
rep('index.html',
  '<div id="modal-root"></div>',
  `<div id="page-root"></div>
<div id="modal-root"></div>`, 'page-root');
rep('index.html',
  '<script src="js/trial.js"></script>',
  `<script src="js/trial.js"></script>
<script src="js/market.js"></script>
<script src="js/page.js"></script>`, 'scripts');
rep('index.html',
  `<div class="set-row" style="gap:6px"><button id="btn-market" class="icon-btn" style="flex:1">坊 市</button><button id="btn-friends" class="icon-btn" style="flex:1">道友录</button></div>`,
  `<div class="set-row" style="gap:6px"><button id="btn-map" class="btn-primary" style="flex:1">地  图</button></div>
  <div class="set-row" style="gap:6px"><button id="btn-market" class="icon-btn" style="flex:1">坊 市</button><button id="btn-friends" class="icon-btn" style="flex:1">道友录</button></div>`, 'map-btn');

/* 2) ui.js：委托加 btn-map；丹房按钮/服丹改跳页面；三页注册 */
rep('js/ui.js',
...
/* 丹房弹窗改为页面：showPillHouse 直接跳页，原弹窗函数改名复用 */
rep('js/ui.js',
  ...
  function showPillHouseModal() {
    ...
    const { card } = makeModal(removeModals);
    const eco = g.LS.economy;
    const render = () => {`, 'pillhouse-ren');

/* 3) 三页注册（showXinmo 前插入 registerPages） */
rep('js/ui.js',
